...
interface Props {
  ...
  /** Custom message */
  message?: string;
  /** Link to a specific feature page instead of the main backstage-pass page */
  featureHref?: string;
}
 ...
 */
export default function BackstagePassGate({ children, feature, showTeaser, message, featureHref }: Props) {
    ...
    <div className="relative">
      ...
      <div className={`${showTeaser ? "absolute inset-0" : ""} flex flex-col items-center justify-center py-16 px-4`}>
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-8 max-w-md text-center shadow-2xl">
          ...
          <Link
            href={featureHref ?? "/backstage-pass"}
          ...
          >
            ...
            {featureHref ? "Learn More" : "Get the Backstage Pass"}
          </Link>
