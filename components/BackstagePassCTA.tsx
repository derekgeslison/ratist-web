    ...
    <div className="bg-[var(--surface)] border border-amber-400/30 rounded-2xl p-8 text-center">
      {hasPass ? (
        ...
        <>
          ...
          {showNativeUi ? (
            // Native (iOS + Android): open the system browser on the
            // subscribe.theratist.com subdomain. That host is NOT in
            // either app-link manifest, so the OS hands the URL to
            // the system browser instead of routing it back into the
            // WebView. A Next.js host-rewrite in next.config.ts
            // serves the /backstage-pass content while leaving the
            // URL bar pointed at subscribe.theratist.com, so the OS
            // can't re-capture the request mid-flight. External-
            // purchase route (matches what other reader apps do
            // successfully under Apple Guideline 3.1.3).
            <>
