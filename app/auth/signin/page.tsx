...
export default function SignInPage() {
  ...
  // After a Google redirect, the user lands back on this page already signed in.
  // Route them to the right place based on whether they're a new user.
  useEffect(() => {
    if (!authLoading && user) {
      const wasPending = sessionStorage.getItem("ratist_google_redirect_pending") === "1";
      if (wasPending) {
        sessionStorage.removeItem("ratist_google_redirect_pending");
        const isNew = sessionStorage.getItem("ratist_new_user") === "1";
        sessionStorage.removeItem("ratist_new_user");
  ...
  async function handleGoogle() {
    setError("");
    setLoading(true);
    // Mark that we're initiating a redirect — used above to route after returning
    sessionStorage.setItem("ratist_google_redirect_pending", "1");
    try {
      await signInWithGoogle();
      // Browser navigates to Google — code below this line won't run
    } catch (e: unknown) {
      sessionStorage.removeItem("ratist_google_redirect_pending");
