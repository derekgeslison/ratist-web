import { MovieGridSkeleton } from "@/components/MovieCardSkeleton";

// /watchlist has a list-selector + filter strip + grid. Skeleton
// mirrors the top-of-page chrome and then a poster grid so the user
// gets immediate structural feedback on first paint.
export default function Loading() {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="animate-pulse space-y-3 mb-6">
        <div className="h-8 w-44 bg-[var(--surface-2)] rounded" />
        <div className="flex gap-2">
          <div className="h-9 w-32 bg-[var(--surface-2)] rounded-lg" />
          <div className="h-9 w-32 bg-[var(--surface-2)] rounded-lg" />
          <div className="h-9 w-28 bg-[var(--surface-2)] rounded-lg" />
        </div>
      </div>
      <MovieGridSkeleton count={14} />
    </div>
  );
}
