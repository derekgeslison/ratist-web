...
export async function GET(req: NextRequest) {
  try {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, name: true, email: true, avatarUrl: true, bio: true, isPrivate: true, autoDateOnSeen: true, publicTabs: true, notificationPrefs: true },
    });
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { name, avatarUrl, bio, isPrivate, autoDateOnSeen, publicTabs, notificationPrefs } = await req.json();
    ...
    const updated = await prisma.user.update({
      where: { id: dbUser.id },
      data: update,
      select: { id: true, name: true, avatarUrl: true, bio: true, isPrivate: true, autoDateOnSeen: true, publicTabs: true, notificationPrefs: true },
    });
