...
export default function RecommendPage() {
  ...
  function handleSeeMore() {
    if (visibleCount < results.length) setVisibleCount((v) => v + 10);
    else if (currentPage < totalPages) { fetchResults(currentPage + 1, true); setVisibleCount((v) => v + 10); }
  }
  ...
  function handleStartOver() {
    ...
    setEra(""); setExcludeGenres(new Set()); setResults([]); setHasSearched(false);
    setVisibleCount(5); setSortMode("");
  }
  ...
  const sorted = sortMode === "rating" ? [...results].sort((a, b) => b.voteAverage - a.voteAverage)
    : sortMode === "match" ? [...results].sort((a, b) => (b.matchScore ?? 0) - (a.matchScore ?? 0))
    : results;
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => isLastStep ? handleSubmit() : setStep((s) => s + 1)}
