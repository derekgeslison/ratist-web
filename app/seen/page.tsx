...
import { Eye, Search, Calendar, ArrowUpDown } from "lucide-react";
...
export default function SeenPage() {
  ...
  const [yearMode, setYearMode] = useState<"released" | "watched">("released");
  ...
  const filtered = [...movies
    ...
    .filter((m) => !yearFilter || (
      yearMode === "released"
        ? m.year === yearFilter
        : new Date(m.watchedDate ?? m.seenAt).getFullYear().toString() === yearFilter
    ))
  ].sort((a, b) => {
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-1 text-[10px] text-[var(--foreground-muted)]">
                  <button
                    onClick={() => setYearMode("released")}
                    className={`px-1.5 py-0.5 rounded transition-colors ${yearMode === "released" ? "text-white bg-[var(--surface-2)]" : "hover:text-white"}`}
                  >
                    Release
                  </button>
                  <span>/</span>
                  <button
                    onClick={() => setYearMode("watched")}
                    className={`px-1.5 py-0.5 rounded transition-colors ${yearMode === "watched" ? "text-white bg-[var(--surface-2)]" : "hover:text-white"}`}
                  >
                    Watched
                  </button>
