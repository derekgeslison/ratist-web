...
export const metadata: Metadata = { title: "Movie Maps", description: "Chronological watch orders, themed film journeys, essential viewing guides, and curated lists of movies to watch — mapped out for cinephiles.", alternates: { canonical: "/movie-maps" } };
...
export default async function MovieMapsPage({ searchParams }: { searchParams: Promise<{ sort?: string; q?: string }> }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        <form action="/movie-maps" method="get" className="relative flex-1 w-full sm:w-auto">
        ...
        {q && (
          <Link href={`/movie-maps${sort !== "newest" ? `?sort=${sort}` : ""}`} className="text-sm text-[var(--foreground-muted)] hover:text-white transition-colors">
            Clear search
          </Link>
        )}
        <Suspense>
