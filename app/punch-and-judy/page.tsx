...
export const metadata: Metadata = { title: "Punch & Judy", description: "Head-to-head debates, comparisons, and contrarian takes." };
import Link from "next/link";
import Image from "next/image";
import { Swords, Calendar, Eye, MessageCircle, Search } from "lucide-react";
...
export default async function PunchAndJudyPage({ searchParams }: { searchParams: Promise<{ sort?: string; q?: string }> }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        <form action="/punch-and-judy" method="get" className="relative flex-1 w-full sm:w-auto">
        ...
        {q && (
          <Link href={`/punch-and-judy${sort !== "newest" ? `?sort=${sort}` : ""}`} className="text-sm text-[var(--foreground-muted)] hover:text-white transition-colors">
            Clear search
          </Link>
        )}
        <Suspense>
      ...
      {posts.length === 0 ? (
        <p className="text-[var(--foreground-muted)] text-center py-20">No posts yet. Check back soon.</p>
      ) : (
        <>
